      ...
      try {
        ...
        filteredUsers.forEach((u: any) => {
          ...
          for (let i = 0; i < 6; i++) {
                ...
                if (doneAt) {
                  ...
                  if (match) {
                     ...
                     if (finalDeadline && isTimePastDeadline(doneHM, finalDeadline)) {
                        status = 'terlewat'; // Past final deadline means missed/tidak mengisi
                     } else if (idealDeadline && isTimePastDeadline(doneHM, idealDeadline)) {
                        status = 'terlewat'; // Past ideal deadline but before final deadline means late
                     }
                  }
                }
              } else {
            ...
            if (r.includes('walas')) {
              addSingleTask('Absensi siswa binaan pada pagi hari', 'walas_absenPagi', '08:00', '17:00', () => {
                ...
                if (match) return { completed: true, completedAt: match.created_at || match.timestamp || match.date || match.tanggal }; return null;
              });
            ...
            if (r.includes('guru') || r.includes('guru_mapel')) {
              ...
              uniqueMap.forEach((s: any) => {
                ...
                addSingleTask(`Jurnal Ajar ${s.class_name} (${s.subject_name})`, 'guru_jurnal', deadline, '17:00', () => {
                  ...
                  const match = laporanHarian.find((lh: any) => { if (!(String(lh.tanggal).startsWith(filterDateStr) || String(lh.date).startsWith(filterDateStr) || String(lh.created_at).startsWith(filterDateStr))) return false; let act: any = {}; try { act = typeof lh.activity === 'string' && lh.activity.startsWith('{') ? JSON.parse(lh.activity) : {}; } catch (e) {} const lhClass = act.class || lh.class_name; const lhSubject = act.subject || lh.subject_name; return lhClass === s.class_name && lhSubject === s.subject_name; });
                  if (match) return { completed: true, completedAt: match.created_at || match.timestamp || match.date || match.tanggal }; return null;
                });
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <Card className="w-full max-w-5xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <CardHeader className="py-4 px-6 bg-slate-50 border-b border-slate-100 flex flex-row items-center justify-between shrink-0">
          ...
          <Button variant="ghost" size="sm" onClick={onClose} className="rounded-full hover:bg-slate-200 h-8 w-8">
            <X className="w-4 h-4" />
          </Button>
        </CardHeader>
