...
async function testPoolAndInit() {
  if (pool) {
    try {
      ...
      console.error('Warning: Database connection failed during startup initialization:', e.message);
      console.error('The server will continue running and will try to reconnect on the next request.');
      // Do NOT set pool = undefined; let it retry later.
    }
  }
}
...
export const app = express();
  ...
  app.get('/api/health', async (req, res) => {
    try {
      if (!pool) {
         return res.status(500).json({ status: 'error', message: 'Pool is undefined. DB Connection failed during startup.' });
      }
      await pool.query('SELECT 1');
  ...
  app.all('/api/keyval.php', async (req, res) => {
    try {
      ...
      // Key Value logic using SQL pool
  ...
  app.post(['/api/crud/:table', '/api/data/:table'], async (req, res) => {
    ...
    try {
      ...
      if (!pool) return res.json({ insertId: 1 });
  ...
  app.put(['/api/crud/:table/:id', '/api/data/:table/:id'], async (req, res) => {
    ...
    try {
      ...
      const keys = Object.keys(data);
      const values = Object.values(data);
      if (!pool) return res.json({ affectedRows: 0 });
      // Special handling for class update to sync walas assignment
      if (table === 'classes') {
        // Fetch old class data
        const [oldClassRows]: any = await pool.query('SELECT name, wali_kelas_id FROM classes WHERE id = ?', [id]);
        if (oldClassRows.length > 0) {
           const oldClassName = oldClassRows[0].name;
           const oldWaliKelasId = oldClassRows[0].wali_kelas_id;
           
           // If wali_kelas_id is changing, or name is changing, clear the old user
           if (oldWaliKelasId && (data.wali_kelas_id !== oldWaliKelasId || data.name !== oldClassName)) {
              await pool.query('UPDATE users SET class_name = NULL WHERE id = ?', [oldWaliKelasId]);
           }
        }
      }
      ...
      // Update new walas
      if (table === 'classes' && data.wali_kelas_id) {
         await pool.query('UPDATE users SET class_name = ? WHERE id = ?', [data.name || req.body.name, data.wali_kelas_id]);
      }
  ...
  app.delete(['/api/crud/:table/:id', '/api/data/:table/:id'], async (req, res) => {
    ...
    try {
      if (!pool) return res.json({ affectedRows: 0 });
      if (!pool) return res.json({ affectedRows: 0 });
      // Special handling for class deletion to remove walas assignment
      if (table === 'classes') {
        const [classRows]: any = await pool.query('SELECT name, wali_kelas_id FROM classes WHERE id = ?', [id]);
        if (classRows.length > 0) {
          const className = classRows[0].name;
          const waliKelasId = classRows[0].wali_kelas_id;
          
          // Clear the user's class assignment
          if (waliKelasId) {
             await pool.query('UPDATE users SET class_name = NULL WHERE id = ?', [waliKelasId]);
          } else {
             // Fallback: clear by class name just in case
             await pool.query('UPDATE users SET class_name = NULL WHERE class_name = ?', [className]);
          }
        }
      }
  ...
  app.all(['/api/announcements', '/api/announcements.php'], async (req, res) => {
    try {
      const method = req.method;
      if (method === 'GET') {
        const [rows] = await pool.query('SELECT id, title, content, target_audience as target, created_at as date, created_by FROM announcements ORDER BY created_at DESC') as any;
        // Format rows for React
        const formatted = rows.map((r: any) => {
          let t = r.target || 'Semua';
          if (t === 'semua') t = 'Semua';
          if (t === 'guru') t = 'Guru';
          if (t === 'ortu') t = 'Wali Murid';
          return {
            ...r, category: 'Informasi',
            target: t,
            id: String(r.id),
            isPublished: true
          };
        ...
        const { title, content, target, category, isPublished } = req.body;
        const [result] = await pool.query(
          'INSERT INTO announcements (title, content, target_audience, created_by) VALUES (?, ?, ?, ?)',
          [title, content, target, 1] // Assuming admin ID 1 for now
        ) as any;
        res.json({ status: 'success', id: String(result.insertId) });
      } else if (method === 'PUT') {
        const { id, title, content, target, category, isPublished } = req.body;
        await pool.query(
